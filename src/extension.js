			...
			panel.webview.onDidReceiveMessage(async (message) => {
				if (message.command === 'prompt') {
					while (true) {
						...
						if (newVariableName.includes(' ')) newVariableName = newVariableName.split(' ').join('_');
...
function getWebviewContent(webview, extensionUri) {
	...
	const sidebarStyleURI = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'sidebar.css'));
	...
	const playBtnURI = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'play-circle-outline.svg'));

	const trashBtnURI = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'trash-bin-outline.svg'));
