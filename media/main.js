(function () {
  ...
  const MATH_WHITELIST_BLOCKS = ["math", "get-variable", "call-void"];
  ...
  function getVariables(block) {
    const variables = [];
    const currentProcedureId = block.dataset.currentProcedure;

    while (!block.classList.contains("code-blocks")) {
      if (block.dataset.procedureType === "no-return" || block.dataset.procedureType === "return") break;

      if (block.innerText === "return") {
        return getVariablesPro(currentProcedureId);
      }
  ...
  function getVariablesPro(procedureId) {
    const variables = [];
    const procedureEls = [...document.querySelector(`[data-procedure-id="${procedureId}"] .procedure`).children];
    
    procedureEls.forEach(block => {
      if (block.dataset.variableType === "create-variable") {
        const dtype = block.querySelector('[name="datatype"]').value;
        const name = block.querySelector(".variable-name").value;
        const variableId = block.dataset.variableId;
